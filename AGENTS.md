...
- Phase 8 must produce a production-separated release candidate and a simple PC/Android/iPhone preview route, but must not merge main, replace the current Pages root, modify the current OneDrive file, replace production enemy data/localStorage, enable production workflows or zero-operation updates, or connect a live external-data adapter. Stop for owner review before Phase 9.
