...
globalThis.addEventListener('message', async (event) => {
  ...
  if (accepted) {
    ...
    try {
      saved = JSON.parse(event.data.package.payload.dokkan_calc_data_v22 || '{}');
    } catch {}
    try {
      criticalOverrides = JSON.parse(event.data.package.payload.dokkan_crit_overrides || '{}');
    } catch {}
    summary = describeImportedStorage(saved, criticalOverrides);
  }
  ...
  if (summary) {
    ...
    details.textContent = `移行した内容：保存キャラクター ${summary.characters}件${names}、保存済み状況 ${summary.savedScenarios}件、作業中の状況 ${summary.currentScenarios}件、手動敵 ${summary.manualEnemies}件、設定 ${summary.settings}分類、会心補正 ${summary.criticalOverrides}件。GitHub PATは0件です。イベント・ステージ・配布敵データは増えていません。下のボタンから確認版を開き、「計算する状況」に表示される作業中カードを確認してください。`;
  }
