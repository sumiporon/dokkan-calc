/** Frozen fixture controls adapted only for the one-event live review gate. */
...
async function analyze() {
  ...
  try {
    ...
    if (result.pageKind === 'event') {
      const plan = liveGatePlan(result.material, result.coverage.visitUnits);
      const existing = await send({ type: 'session:get' });
      if (existing) { show('この検証は開始済みです', '元のstageタブで再開してください。別event・再取得は開始しません。'); return; }
      show('eventを確認しました', `${result.material.eventName}・先頭${plan.length}stageのみ検証（reviewまで）`, [{
        label: '開始', click: async () => {
          if (busy) return; busy = true; show('保存中', 'eventと訪問計画を端末へ保存しています。', [{ label: '準備中', disabled: true }]);
          try {
            await send({ type: 'session:start', eventMaterial: result.material, plan, adapterVersion: ONE_TAP_ADAPTER_VERSION });
            location.assign(plan[0].url);
