/**
 * OneDriveロック回避版: C:\Temp にコピーしてパッチ適用後、戻す
 */
const fs = require('fs');
...
if (content.includes(oldCalcCrit)) {
    ...
    changeCount++;
    console.log('   PATCH 1 OK: calculateNewDurability 修正完了');
} else {
    console.error('   PATCH 1 FAIL: 対象コードが見つかりません');
}
...
if (content.includes(oldRenderCrit)) {
    ...
    changeCount++;
    console.log('   PATCH 2 OK: renderDynamicAttacks 修正完了');
} else {
    console.error('   PATCH 2 FAIL: 対象コードが見つかりません');
}
