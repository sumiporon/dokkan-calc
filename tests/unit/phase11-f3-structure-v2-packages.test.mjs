...
import { ARCHIVE, OWN_SOURCES, validateManifest, buildCandidate } from '../../amo/phase11-f3-structure-v2-reviewer/build.mjs';
import { writeZip } from '../../amo/phase11-f3-structure-v2-reviewer/zip.mjs';

const root = path.resolve('.'), out = path.join(root, 'generated/phase11-f3-structure-extension/v2/signing');
...
test('reviewer source allowlist contains original source, pinned minimal dependencies and exact expected hash', async () => {
  const zip = entries(await readFile(path.join(out, 'phase11-f3-structure-diagnostic-v2-0.0.2-reviewer-source.zip')));
  const templates = ['README.md', 'package.json', 'package-lock.json', 'build.mjs', 'zip.mjs', 'verify.mjs'];
  const manifest = 'prototypes/phase11-f3-structure-v2-extension/manifest.json';
  ...
  for (const name of templates) assert.deepEqual(zip.get(name), await readFile(path.join(root, 'amo/phase11-f3-structure-v2-reviewer', name)));
...
test('packaged v2 bundle is byte-identical to the approved fixture-build recipe output', async () => {
  const ordinary = path.join(root, 'generated/phase11-f3-structure-extension/v2/candidate');
  for (const name of ['content.js', 'manifest.json']) assert.deepEqual(await readFile(path.join(out, 'candidate', name)), await readFile(path.join(ordinary, name)));
  assert.equal(JSON.parse(await readFile(path.join(out, 'candidate/manifest.json'), 'utf8')).version, '0.0.2');
});
