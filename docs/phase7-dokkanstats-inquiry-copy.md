# 第7段階 DokkanStats問い合わせコピー用（未送信）

作成日: 2026-08-24（JST）

**Codexは送信していない。project ownerが自分で送信する。返信を受け取るまでDokkanStatsの自動取得やadapter実装を開始しない。**
...
以下の`Hello DokkanStats team,`から最後のURLまでを、そのままemail本文へコピーする。
...
返信は公開repositoryへそのまま載せず安全な場所へ保存し、取得を始める前に全文をCodexへ提示する。`access`、`history`、`storage`、`transformation`、`redistribution`、`automation`、`coverage`、`freshness`、`lifecycle`ごとに`allowed`、`denied`、`unknown`としてpermission ledgerへ反映する。曖昧な回答では開始しない。
