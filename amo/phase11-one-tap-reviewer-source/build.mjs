...
await cp(path.join(SRC, 'manifest.fixture.json'), path.join(OUT, 'manifest.json'));
await cp(path.join(SRC, 'review.html'), path.join(OUT, 'review.html'));
...
await build({
  ...
  legalComments: 'none',
  banner: { js: 'const browser=globalThis.browser??globalThis.chrome;' },
  define: { PHASE11_FIXTURE_MODE: 'true' }
});
...
const archive = path.join(DIST, 'phase11-fixture-amo-upload-v2.zip');
