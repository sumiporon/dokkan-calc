...
export const ARCHIVE = 'phase11-f3-structure-diagnostic-v2-0.0.2-unsigned.zip';
export const OWN_SOURCES = [
  'prototypes/phase11-f3-structure-v2-extension/content.mjs',
  'prototypes/phase11-f3-structure-v2-extension/ui.mjs',
  'src/prototype/phase11-f3-structure-v2.mjs',
...
export function validateManifest(manifest) {
  ...
  assert.equal(manifest.manifest_version, 3); assert.equal(manifest.version, '0.0.2');
  ...
  assert.equal(manifest.description, 'One-shot bounded structure diagnostic v2. Results remain in page memory only.');
...
export async function buildCandidate(root, out, { archive = true } = {}) {
  ...
  const manifest = JSON.parse(await readFile(path.join(root, 'prototypes/phase11-f3-structure-v2-extension/manifest.json'), 'utf8'));
